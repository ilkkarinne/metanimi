...
com.metanimi.VideoAnimator = (function() {
	...
	function init(containers) {
		...
		_buf1 = $('<video></video>').attr({
			'style': 'width:' + _width + 'px;height:' + _height + 'px;display:none;',
			'id': 'buf1'
		});
		_buf2 = $('<video></video>').attr({
			'style': 'width:' + _width + 'px;height:' + _height + 'px;display:none;',
			'id': 'buf2'
		});
		...
		_dayIndicator = $('<div></div>').css({
			'z-index': 100,
			'font-family': 'Helvetica, Arial, sans-serif',
			'font-size': '13px',
			'color': '#848484'
		}).addClass('dayindicator');
		...
		var videoEvents = {
			...
			'progress': function(event) {
				var buf = $(event.target);
				var index = parseInt(buf.attr('data-clipindex'));
				var ranges = buf.get(0).buffered.length;
				_bufferedFractions[index] = [];
				var i = 0;
				var dur = buf.get(0).duration;
				for (i = 0; i < ranges; i++) {
					_bufferedFractions[index].push({
						start: Math.min(buf.get(0).buffered.start(i) / dur, 1),
						end: Math.min(buf.get(0).buffered.end(i) / dur, 1)
					});
				}
			}
		};
	...
	function initClock(clock) {
		...
		_clockFace = _clockPaper.circle(50, 50, 45).attr({
			"fill": "#ffffff",
			"stroke-width": 0
		});
		_clockLoadedPie = _clockPaper.set();
		_clockRim = _clockPaper.circle(50, 50, 45).attr({
			"fill-opacity": 0,
			"stroke": "#848484",
			"stroke-width": "4"
		});
		...
		$.ajax({
			...
			success: function(data) {
				...
				_.each(_clips, function(clip, cIndex) {
					...
					_clips[cIndex].clipDayOffset = _clips[cIndex].start.diff(moment(_clips[cIndex].start).local().startOf('day')) / (1000 * 60 * 60 * 24);
					_clips[cIndex].clipDayFraction = _clips[cIndex].duration / (1000 * 60 * 60 * 24);
				});
	...
	function updateThumbnail(t) {
		...
		if (_thumbnail.css('background-image') != 'url(' + url + ')') {
			_thumbnail.css({
				'background-image': 'url(' + getMonthThumbsUrl(t) + ')',
				'background-position': getMonthThumbSpritePos(t) + 'px 0px'
			});
		}
		_thumbnail.animate({
			...
			step: function(now, fx) {
				$(fx.elem).css({
					'background-position': now + 'px 0px'
				});
			},
			duration: 500
		});
	...
	function updateClock() {
		...
		var clipDayFraction = _clips[_clipIndex].clipDayFraction;
		var clipDayOffset = _clips[_clipIndex].clipDayOffset;
		_.each(_bufferedFractions[_clipIndex], function(frac) {
			_clockLoadedPie.push(createSVGSector(_clockPaper, 50, 50, 45, (frac.start * clipDayFraction + clipDayOffset) * 360, (frac.end * clipDayFraction + clipDayOffset) * 360, {
				"fill": "#fafafa",
				"stroke-width": 0
			}));
		});
