...
function updateIndex(){
	local VIDEO_FILES=`aws --profile metanimi s3 ls ${S3_VIDEO_BASE}/ | grep ".webm" | awk '{print $4}'`
	local PARTS=
	local OUT='{"clips": ['
	local START=
	local END=
	for FNAME in ${VIDEO_FILES}; do
		PARTS=(${FNAME//_/ })
		START=`date -ju -v0S -f "%Y-%m-%dT%H-%M" ${PARTS[4]}T${PARTS[5]%\.webm} +%s`
	...
	aws --profile metanimi s3 cp ${VIDEO_BASE}/index.json ${S3_VIDEO_BASE}/ --no-guess-mime-type --content-type "application/javascript" --acl public-read --cache-control "max-age=0, must-revalidate"
	return $?
}
...
END_STAMP=`date -ju -f '%s' ${END} +%Y-%m-%dT%H:%M:%S`.000Z
./load_daily.sh --start ${START_STAMP} --end ${END_STAMP} --fast
RETVAL=$?
if [ ! ${RETVAL} -eq 0 ]; then
	exit ${RETVAL}
fi
...
then
	...
	OUT=${VIDEO_BASE}/${OUT1}'_'${OUT2}'_'${OUT3}.webm
	...
	ffmpeg -v quiet -y -loop 1 -i ${BASE} -i ${TMP}/${FRAME_BASE_NAME} -i ${OVERLAY} -filter_complex "overlay [tmp]; [tmp] overlay" -c:v libvpx -crf 10 -b:v 4M -c:a copy -vframes ${FRAMES} -f webm ${OUT}
	RETVAL=$?
	if [ ! ${RETVAL} -eq 0 ]; then
		exit ${RETVAL}
	fi
...
./create_month_thumbs.sh --month `date -j -f '%s' ${END} +%Y-%m`
RETVAL=$?
if [ ! ${RETVAL} -eq 0 ]; then
